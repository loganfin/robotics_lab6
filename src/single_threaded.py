...
class FanucSingle(Node):
    def __init__(self, namespace):
        ...
        self.joints_ac = ActionClient(
            self, JointPose, f"/{namespace}/joint_pose"
        )
    ...
    def demo(self):
        ...
        #self.cart_ac.send_goal_async(cart_goal).add_done_callback(self.goal_response_callback)

        self.joints_ac.wait_for_server()  # Wait till its ready
        joint_goal = JointPose.Goal()  # Make Goal
        # Add all joints
        joint_goal.joint1 = 0.0
        joint_goal.joint2 = -1.7
        joint_goal.joint3 = -31.9
        joint_goal.joint4 = 0.0
        joint_goal.joint5 = -58.5
        joint_goal.joint6 = -68.3
        # send_goal_async is nonblocking and allows us to get feedback when in a single thread
        future = self.joints_ac.send_goal_async(
            joint_goal, feedback_callback=self.feedback_callback
        )
        future.add_done_callback(
            self.goal_response_callback
        )  # This will run when the server accepts the goal
    ...
    def goal_response_callback(self, future):
        ...
        )  # This will run when the goal is done
    ...
    def feedback_callback(
        self, feedback_msg
    ):  # This function will run when feedback is recieved from the server
        feedback = feedback_msg.feedback
        self.get_logger().info(
            "Received feedback: {0}".format(feedback.distance_left)
        )
